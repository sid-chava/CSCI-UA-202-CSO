...
int main()
{
  ...
  for(i=0; i < (L2_NUM_CACHE_ENTRIES * WORDS_PER_CACHE_LINE); i += WORDS_PER_CACHE_LINE) {
    ...
    // the address of the cache line to be written to is i * BYTES_PER_WORD (which is 8)
    l2_cache_access(i << WORDS_TO_BYTES_SHIFT, write_data, WRITE_ENABLE_MASK, NULL, &status);

    if (!(status & 0x1)) { //if cache miss, indicated by lsb of status = 0
      //call l2_insert_line
      l2_insert_line(i << WORDS_TO_BYTES_SHIFT, write_data, &evicted_writeback_address, 
  ...
  for(i=0; i < (L2_NUM_CACHE_ENTRIES * WORDS_PER_CACHE_LINE); i += WORDS_PER_CACHE_LINE) {
    ...
    l2_cache_access(i << WORDS_TO_BYTES_SHIFT, NULL, READ_ENABLE_MASK, read_data, &status);
  ...
  for(i = L2_NUM_CACHE_ENTRIES * WORDS_PER_CACHE_LINE; 
	...
	printf("Should have read %llu but read %llu instead\n", ((i+j) << 2), read_data[j]);
	exit(1);
    }
  }
